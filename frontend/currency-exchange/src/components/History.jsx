  ...
  return (
    <div className="bg-white p-4 md:h-screen rounded-2xl shadow">
      ...
      <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4 mb-4">
        <button
          className={`p-4 rounded-3xl transition-all duration-400 ${activeTab === 'Data' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          onClick={() => setActiveTab('Data')}
        >
          Data
        </button>
        <button
          className={`p-4 rounded-3xl transition-all duration-400 ${activeTab === 'Chart' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
